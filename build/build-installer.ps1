# Genera el instalador de AutoEscritorio con Inno Setup. Reconstruye el dist por
# defecto; -SkipBuild lo reutiliza. Version = APP_VERSION (fuente unica).
param([switch]$SkipBuild)

$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $PSScriptRoot
$distExe = Join-Path $root "dist\AutoEscritorio\AutoEscritorio.exe"
...
$initPy = Join-Path $root "autoescritorio\__init__.py"
...
& $iscc "/DMyAppVersion=$ver" (Join-Path $PSScriptRoot "AutoEscritorio.iss")
if ($LASTEXITCODE -eq 0) {
    Write-Host "`nInstalador: $(Join-Path $root ("installer\AutoEscritorio-Setup-$ver.exe"))" -ForegroundColor Green
} else {
